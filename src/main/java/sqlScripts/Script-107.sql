...
from (
	select t1.id, t1.plan_id, t1.name, t1.type, dev1.name as deviceName
		...
		, t1.post_id, t1.classes_id, tr.flight, tr.vsl_name, tr.start_time as berthingTime
		, tr.end_time as unberthingTime
		, IF(t1.event_status_name is null
			or t1.event_status_name = '', ref3.data_name, t1.event_status_name) as eventStatusName
		, relat.code as relationshipCode, pty5.name as departmentName, t5.name as vehicleType, t5.car_number, t5.tonnage
		, t5.certificate, t5.producer, t5.model, t5.brand, f.name as formName
		, pla.plan_name as planName, p4.name as assignerPersonName, cate.name as deviceCategoryName, cate.classify as classify, typ.name as deviceTypeName
		, dev1.id as device, IFNULL(t1.last_updated_time, t1.created_time) as lastUpdatedTime, t1.created_time
		...
		, teams.name as teamName, work.proc_instance_id as procInstanceId, work.business_type as taskBusinessType, work.task_id as taskId, sft.task_name as taskName
		, group_concat(DISTINCT sft.task_user_id) as taskUserId, group_concat(DISTINCT act.USER_ID_) as actUserId
		, sft.form_key as formKey, t1.location, t1.approve_routes, t7.result
	from pfm_work t1
		left join work_center work
		on work.business_key = t1.id
		...
		left join SF_TASK_INSTANCE sft
		...
		left join ACT_RU_IDENTITYLINK act on sft.task_code = act.TASK_ID_
		left join pty_party p1 on p1.id = t1.complete_person
		left join pty_party p4 on p4.id = t1.assigner_person
		...
		left join pty_party pty7 on pty7.id = t1.review_id
		left join pty_party pty8 on pty8.id = t1.cancel_id
	where 1 = 1
	group by t1.id
) t11
where 1 = 1
	and 3 = 3
	and (t11.party_group_id in (27009392, 27009386, 27009393, 27009394)
		or t11.party_group_id is null)
	and t11.type in ('periodicity', 'trigger')
	and t11.status in ('created', 'distributed')
	and t11.org_id = 27009365
	and t11.business_type in ('patrol', 'operationInspection', 'periodicInspection')
