...
from
	...
	select
		...
		t1.approve_routes,
		t7.result,
		max(spa.object_id) as objectId
	from
		pfm_work t1
	left join (
	select * from 
	work_center wc1
	where 	1 = 1	and wc1.business_type in ('ope_inspection_work', 'trigger_inspection_work', 'patrol_special_inspection_work', 'patrol_temporary_inspection_work', 'specialOperationPatrol', 'periodicInspection')
		and wc1.object_state = 'enabled'
	) work on
		work.business_key = t1.id
	left join (
	select * from 
	SF_TASK_INSTANCE sti
	where 1 = 1
			and sti.business_type in ('ope_inspection_work', 'trigger_inspection_work', 'patrol_special_inspection_work', 'patrol_temporary_inspection_work', 'specialOperationPatrol', 'periodicInspection')
		and sti.status = 'approving'
	) sft on
	...
	left join dev_inspection_form dif on
		dif.work_id = t1.id
	left join sche_plan_association spa on
		spa.business_key = dif.id
	where
		1 = 1
-- 				and t1.assigner_person = 684001577
	group by
		t1.id ) t11
where
	1 = 1
	and 3 = 3
-- 	and t11.status = "created"
-- 	and t11.org_id = 27009365
	and t11.classify in ( "otherDevice" , "specialDevice" , "safetyDevice" )
	and t11.type in ("spotCheck")
order by
	t11.created_time desc 
	limit 1,30
	;

 
select t1.assigner_person, t1.due_status from pfm_work t1
where 1 = 1
and t1.due_status is not null
group by t1.due_status