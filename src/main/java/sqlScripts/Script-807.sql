...
from
	...
	select
		...
		tr.workbill_no as eventWorkbillNo,
		tr.work_type_nam as eventWorkTypeName,
		tr.proc_nam as eventProcName,
		tr.way_nam as eventWayName,
		tr.bag_nam as eventBagName,
		tr.cargo_txt as cargoTxt,
...
left join (
	...
	from
			SF_TASK_INSTANCE stii
	where
			1 = 1
		and stii.status = 'approving'
		and stii.business_type in ('ope_inspection_work', 'trigger_inspection_work', 'patrol_special_inspection_work', 'patrol_temporary_inspection_work', 'specialOperationPatrol', 'periodicInspection')
	group by
			stii.business_key ) sft on
		sft.business_key = t1.id
left join (
	select
			arii.*,
			group_concat(distinct arii.USER_ID_) as actUserId
	from
			ACT_RU_IDENTITYLINK arii
			) act on
...
left join pty_party pty8 on
	...
	and pty7.type = 'person'
left join ope_special_operation opeo on
		opeo.id = t1.business_key
	and t1.business_type = 'specialOperationPatrol'
where
		1 = 1
	and relat.code like '001010%'
		and (
	t1.business_type = 'patrol' or t1.business_type = 'operationInspection' or t1.business_type = 'deviceInspection'
	)
-- 	and t1.cabin_flag is null
group by
		t1.id ) t11
where
	1 = 1
	and 3 = 3
-- 		and t11.business_type in ( 'patrol' , 'operationInspection' , 'deviceInspection' )
