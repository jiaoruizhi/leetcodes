...
left join ref_reference ref7 on
	ref7.element_code = 'issueLevelNum'
	and t1.level_num = ref7.data_code
...
where
	...
	select t1.id from ins_issue t1
...
-- 	and t1.status != 'draft'
	...
	and t1.status != 'beConfirmed' and t1.object_state = 'enabled'
	and (t2.object_state = 'enabled' or t1.external_code is not null)
	and relat.code like '001010029005%'
	and relat.status = 'enabled' order by t1.created_time desc ) )b 
	where b.id = t1.id
	)
