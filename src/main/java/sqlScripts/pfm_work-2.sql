...
left join pty_relationship relat on
	...
	-- left join pty_relationship relat1 on relat1.party_id_from = t1.party_group_id -- and relat1.role_type_from = 'department' and relat.relationship_type = 'organizational_structure' 
...
left join hc_vehicle t5 on
	t5.id = t4.vehicle_id
	-- left join ref_reference ref1 on ref1.data_code = t1.type and ref1.element_code = 'deviceCheckType' and t1.business_type in ('deviceInspection','fireDeviceInspection') 
...
-- 	and ((relat.code like concat('001010031', '%'))
-- 	or (relat.code = '001010031' ))
	and relat.code like concat( '001010031', '%')
	and ((ref3.data_name like CONCAT('%', '靠泊', '%')
		and t1.event_status_name is null)
	or (t1.event_status_name like CONCAT('%', '靠泊', '%')))
	and (t1.due_status is null
		or t1.due_status != 'overdue')
	and t1.party_group_id in ( 27009386 )
	and t1.business_type in ( 'patrol' , 'operationInspection' )
group by
	t1.id
order by
	t1.id desc
