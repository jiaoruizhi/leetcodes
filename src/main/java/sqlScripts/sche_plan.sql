...
where 1 = 1
	and name like "%日检%"
-- and t1.classes_id = 5000004
-- and t1.region_id = 15
-- and t1.event_status = "SMMS_VSL_WORKING"
-- and t1.org_id = 27009365
-- and t1.classes_id is not null
;


select pty1.name, t1.person_id , t1.* , t1.org_id , t1.work_status_name 
from sche_plan t1
left join pty_party pty1 on pty1.id = t1.person_id
where 1 = 1
and t1.org_id = 27009323
and special_patrol_category = "day"
and t1.org_id is not null
;
...
select t1.classes_id , t1.* from sche_plan t1
where 1 = 1
-- and t1.classes_id = 5000004
-- and t1.region_id = 15
-- and t1.event_status = "SMMS_VSL_WORKING"
...
and DATE_FORMAT(t1.plan_start_date, '%Y-%m-%d') <= '2024-08-01'
and DATE_FORMAT(t1.plan_finish_date, '%Y-%m-%d') >= '2024-08-01'
and t1.event_status like '%SMMS_VSL_ARRIVED%'
and t1.status = true
