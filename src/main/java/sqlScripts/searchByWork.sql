...
from
	...
	left join (
		...
		from
			ACT_RU_IDENTITYLINK arii ) act on
	...
	where
		...
		select
			party_id_from
		from
			pty_relationship pr1
		where
			pr1.code like '001010%'
				and pr1.role_type_from = 'company'
				and pr1.relationship_type = 'organizational_structure')
...
where
	...
	and DATE_FORMAT(t11.start_time, '%Y-%m-%d') >= '2025-01-01'
	and DATE_FORMAT(t11.start_time, '%Y-%m-%d') <= '2025-01-31'
order by
