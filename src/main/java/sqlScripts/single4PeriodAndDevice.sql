




-- single4PeriodAndDevice


-- device

select
	t1.org_id,
	t1.party_group_id,
	t1.type,
	t1.business_type,
	t1.status,
	t1.*,
	count(t1.id) as countValue
from
	pfm_work t1
left join dev_device dev1 on
	dev1.id = t1.business_key
	and t1.business_type in ('deviceInspection', 'fireDeviceInspection')
left join dev_device_category cate on
	cate.id = dev1.category
left join pty_relationship relat on
	relat.party_id_from = t1.org_id
	and relat.role_type_from = 'company'
	and relat.relationship_type = 'organizational_structure'
where
	1 = 1
	and t1.status not in ('retract', 'nullify', 'cancel')
	and relat.code like concat('001010', '%')
	and relat.status = 'enabled'
	and t1.type = 'spotCheck'
	and t1.business_type = 'deviceInspection'
	and DATE_FORMAT(t1.start_time, '%Y-%m-%d') >= '2025-01-01'
	and DATE_FORMAT(t1.start_time, '%Y-%m-%d') <= '2025-01-31'
	and cate.classify in ( 'otherDevice' , 'specialDevice' , 'safetyDevice' )
group by
	t1.id ;






-- period









select
	t1.org_id,
	t1.party_group_id,
	t1.type,
	t1.business_type,
	t1.status
-- 	count(t1.id) as countValue
from
	pfm_work t1
left join pty_relationship relat on
	relat.party_id_from = t1.org_id
	and relat.role_type_from = 'company'
	and relat.relationship_type = 'organizational_structure'
where
	1 = 1
	and t1.status not in ('retract', 'nullify', 'cancel')
	and t1.status is not null
	and relat.code like '001010%'
	and relat.status = 'enabled'
	and t1.business_type = 'periodicInspection'
	and DATE_FORMAT(t1.created_time, '%Y-%m-%d') >= '2025-01-01'
	and DATE_FORMAT(t1.created_time, '%Y-%m-%d') <= '2025-01-31'
-- group by
-- 	t1.status ;





