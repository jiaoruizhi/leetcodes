			...
			left join SF_TASK_INSTANCE sft on
				...
				where
					...
					and ((relat.code like concat('001010031', '%')
						and relat.code != '001010031')
					or (relat.code = '001010031'
						and (t1.party_group_id in ( 27009392 , 27009386 , 27009393 , 27009394 )
							or t1.party_group_id is null) ))
					and relat.status = 'enabled'
					and (t1.due_status is null
						or t1.due_status != 'overdue')
					and t1.type like CONCAT('%', 'special', '%')
						and t1.business_type like CONCAT('%', 'patrol', '%')
					group by
						t1.id
					order by
						t1.id desc
